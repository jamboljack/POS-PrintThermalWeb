                    ...
                    <div class="col-md-6">
                        <div class="portlet box blue-madison">
                            <div class="portlet-title">
                                <div class="caption">
                                    <i class="fa fa-plus-circle"></i> Form Data Stok Masuk
                                </div>
                            </div>
            ...
            <div class="col-md-12">
                <div class="portlet box blue-madison">
                    ...
                    <div class="portlet-body form">
                        ...
                        <input type="hidden" name="masuk_temp_id" id="masuk_temp_id">
...
$(document).ready(function() {
    table = $('#tableData').DataTable({
        ...
        "order": [],
        "ajax": {
            "url": "<?=site_url('admin/masuk/data_temp_list')?>",
            "type": "POST"
        },
        "columnDefs": [
            {
...
$("#btn_simpan").click(function() {
    ...
    $.ajax({
        url: '<?=site_url('admin/masuk/savedata');?>',
        type: "POST",
        data: dataString,
        success: function(data) {
            swal({
    ...
    $("form").validate({
        ...
        submitHandler: function(form) {
            if (statusinput == 'Tambah') {
                dataString = $(".form").serialize();
                $.ajax({
                    url: '<?=site_url('admin/masuk/saveitem');?>',
                    type: "POST",
                    data: dataString,
                    success: function(data) {
                        resetForm();
                ...
                dataString = $(".form").serialize();
                $.ajax({
                    url: '<?=site_url('admin/masuk/updateitem');?>',
                    type: "POST",
                    data: dataString,
                    success: function(data) {
                        resetForm();
...
function hitungTotal() {
    $.ajax({
        url : "<?=site_url('admin/masuk/get_data_total_temp');?>",
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            var Qty      = 0;
...
function editData(id) {
    statusinput = 'Edit';
    $.ajax({
        url : "<?=site_url('admin/masuk/get_data_item/');?>"+id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            var locale      = 'en';
            ...
            $('#masuk_temp_id').val(data.masuk_temp_id);
            $('#barang_id').val(data.barang_id);
            $('#kode_barang').val(data.barang_kode);
            $('#nama_barang').val(data.barang_nama);
            $('#kategori').val(data.kategori_nama);
            $('#qty').val(formatter.format(data.masuk_temp_qty));
            document.formBarang.qty.disabled=false;
            document.formBarang.btn_item.disabled=false;
            $("#btn_reset").attr("disabled", false);
            $('#qty').focus();
...
function hapusData(masuk_temp_id) {
    var id = masuk_temp_id;
    swal({
        title: 'Anda Yakin ?',
        ...
        if (!isConfirm) return;
        $.ajax({
            url : "<?=site_url('admin/masuk/deleteitem')?>/"+id,
            type: "POST",
            success: function(data) {
                resetForm();
                reload_table();
