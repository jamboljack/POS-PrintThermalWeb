                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-5">
                                                <div class="input-icon right">
                                                    <i class="fa"></i>
                                                    <input type="text" class="form-control date-picker" placeholder="DD-MM-YYYY" name="tanggal" id="tanggal" value="<?=date('d-m-Y', strtotime($detail->masuk_tanggal));?>" autocomplete="off" autofocus>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-9">
                                                <div class="input-icon right">
                                                    <i class="fa"></i>
                                                    <input type="text" class="form-control" placeholder="Enter Keterangan" name="keterangan" id="keterangan" value="<?=$detail->masuk_keterangan;?>" autocomplete="off">
                                                </div>
                                            </div>
                                        </div>
            ...
            <div class="col-md-12">
                <div class="portlet box blue-madison">
                    ...
                    <div class="portlet-body form">
                        ...
                        <input type="hidden" name="masuk_detail_id" id="masuk_detail_id">
...
$(document).ready(function() {
    ...
    table = $('#tableData').DataTable({
        ...
        "order": [],
        "ajax": {
            "url": "<?=site_url('admin/masuk/data_detail_list/')?>"+masuk_id,
            "type": "POST"
        },
        "columnDefs": [
            {
...
function dataBarang() {
    ...
    tableBarang = $('#tableDataBarang').DataTable({
        ...
        "order": [],
        "ajax": {
            "url": "<?=site_url('admin/masuk/data_barang_list'); ?>",
            "type": "POST"
        },
        "columnDefs": [
            {
...
function pilihdata(id) {
    $.ajax({
        url : "<?=site_url('admin/masuk/get_data_barang/'); ?>" + id,
        type: "GET",
        dataType: "JSON",
...
$("#btn_simpan").click(function() {
    ...
    $.ajax({
        url: '<?=site_url('admin/masuk/updatedata');?>',
        type: "POST",
        data: dataString,
        success: function(data) {
            swal({
    ...
    $("form").validate({
        ...
        submitHandler: function(form) {
            if (statusinput == 'Tambah') {
                dataString = $(".form").serialize();
                $.ajax({
                    url: '<?=site_url('admin/masuk/saveitemdetail');?>',
                    type: "POST",
                    data: dataString,
                    success: function(data) {
                        resetForm();
                ...
                dataString = $(".form").serialize();
                $.ajax({
                    url: '<?=site_url('admin/masuk/updateitemdetail');?>',
                    type: "POST",
                    data: dataString,
                    success: function(data) {
                        resetForm();
...
function editData(id) {
    statusinput = 'Edit';
    $.ajax({
        url : "<?=site_url('admin/masuk/get_data_detail/');?>"+id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            var locale      = 'en';
            ...
            $('#masuk_detail_id').val(data.masuk_detail_id);
            $('#barang_id').val(data.barang_id);
            $('#kode_barang').val(data.barang_kode);
            $('#nama_barang').val(data.barang_nama);
            $('#kategori').val(data.kategori_nama);
            $('#qty').val(formatter.format(data.masuk_detail_qty));
            $('#qty_lama').val(formatter.format(data.masuk_detail_qty));
            document.formBarang.qty.disabled=false;
            document.formBarang.btn_item.disabled=false;
            $("#btn_reset").attr("disabled", false);
            $('#qty').focus();
...
function hapusData(masuk_detail_id) {
    var id = masuk_detail_id;
    swal({
        title: 'Anda Yakin ?',
        ...
        if (!isConfirm) return;
        $.ajax({
            url : "<?=site_url('admin/masuk/deleteitemdetail')?>/"+id,
            type: "POST",
            success: function(data) {
                swal({
                    title:"Sukses",
