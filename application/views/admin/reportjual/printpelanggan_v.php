...
<title>Print Penjualan per Pelanggan</title>
<style type="text/css">
    table {
        width: 100%;
...
<div class="page">
    <table width="100%" align="center" cellpadding="0" cellspacing="0">
        ...
        <tr>
            <td align="center" valign="top" style="font-size: 15px; font-weight: bold;"><u>PENJUALAN PER PELANGGAN</u></td>
        </tr>
        <tr>
    ...
    <table width="100%">
        <tr>
            ...
            <th width="25%" style="border-top: 0.5px solid black; border-bottom: 0.5px solid black; border-right: 0.5px solid black;">NAMA BARANG</th>
        ...
        foreach($listData as $r) { 
            ...
            if ($dari != 'all' && $sampai != 'all') {
                ...
                $listDetail = $this->db->order_by('penjualan_tanggal', 'asc')->get_where('v_penjualan_detail', array('penjualan_tanggal >=' => $tgl_dari, 'penjualan_tanggal <=' => $tgl_sampai, 'pelanggan_id' => $pelanggan_id))->result();
            } else {
                $listDetail = $this->db->order_by('penjualan_tanggal', 'asc')->get_where('v_penjualan_detail', array('pelanggan_id' => $pelanggan_id))->result();
            }

            if (count($listDetail) > 0) {
        ?>
        <tr>
            <th align="left" colspan="9" style="border-left: 0.5px solid black; border-right: 0.5px solid black;">Pelanggan : <?=$r->pelanggan_nama;?></th>
        </tr>
        <?php
            $no    = 1;
            $total = 0;
