...
class Meja extends MY_Controller
{
    ...
    public function data_list()
    {
        ...
        foreach ($List as $r) {
            ...
            $row     = array();
            $meja_id = $r->meja_id;
            $row[]   = '<a title="Edit Data" href="javascript:void(0)" onclick="edit_data(' . "'" . $meja_id . "'" . ')"><i class="icon-pencil"></i></a>
                            <a onclick="hapusData(' . $meja_id . ')" title="Delete Data"><i class="icon-close"></i></a>';
            $row[]  = $no;
            $row[]  = $r->meja_nama;
            $data[] = $row;
        }
